...
export const useGeneratorStore = defineStore('generator', {
  ...
  actions: {
    ...
    startGeneration() {
      ...
      this.progress.phase = 'queued'
      this.progress.message = '任务正在排队，等待图片服务处理'
      this.taskId = null
      // 为每个页面创建对应的图片占位对象
    ...
    updateProgress(
      ...
      if (image) {
        ...
        if (status !== 'error' && status !== 'interrupted') delete image.error
        // 仅在第一次进入完成态时增加计数，避免重复 SSE 事件导致进度溢出。
    ...
    setGenerationMessage(message?: string, phase?: string) {
      if (message) this.progress.message = message
      if (phase) this.progress.phase = phase
    },
    ...
    interruptPendingImages(message = '状态连接已中断，请稍后确认任务结果') {
      ...
      this.progress.phase = 'interrupted'
     ...
     */
    finishGeneration(taskId: string, failed = 0) {
      this.taskId = taskId
      this.stage = 'result'
      this.progress.status = failed > 0 ? 'error' : 'done'
      this.progress.phase = 'finished'
      this.progress.message = failed > 0
        ? `生成结束，${failed} 张图片失败`
        : '全部图片生成完成'
    },
