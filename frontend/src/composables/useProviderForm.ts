...
export const imageTypeOptions = [
  ...
  { value: 'openai_compatible', label: 'OpenAI 兼容图片接口（推荐）' },
  { value: 'image_api', label: '图片 API（支持参考图/比例参数）' }
]
...
export function useProviderForm() {
  ...
  async function autoSaveConfig() {
    ...
    try {
      ...
      setError(result.error || result.error_message || '保存配置失败', '保存配置失败')
      return false
    } catch (e) {
  ...
  async function saveTextProvider() {
    ...
    if (await autoSaveConfig()) closeTextModal()
  }
  ...
  async function saveImageProvider() {
    ...
    if (await autoSaveConfig()) closeImageModal()
  }
