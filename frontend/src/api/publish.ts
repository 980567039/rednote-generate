...
export async function checkPublishLogin(): Promise<PublishApiResponse> {
  try {
    const response = await axios.post(`${API_BASE_URL}/publish/auth/check`, {}, { timeout: 20000 })
    return normalizePublishResponse(response.data)
  } catch (error) {
    return { success: false, ...getApiErrorPayload(error, '检查小红书登录状态失败') }
  }
}

export async function openPublishLogin(): Promise<PublishApiResponse> {
  try {
    const response = await axios.post(`${API_BASE_URL}/publish/auth/login`, {}, { timeout: 20000 })
    return normalizePublishResponse(response.data)
  } catch (error) {
    return { success: false, ...getApiErrorPayload(error, '打开小红书登录页失败') }
  }
}
