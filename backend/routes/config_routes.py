...
IMAGE_PROVIDER_TYPES = {'google_genai', 'openai_compatible', 'image_api'}


class ConfigValidationError(Exception):
    """内部参数校验异常，保留给路由转换为统一的 400 响应。"""

    def __init__(self, detail: str, suggestion: str):
        self.error = validation_error(detail, suggestion)
...
def create_config_blueprint():
    ...
    def get_config():
        ...
        try:
            ...
            return api_error_response(e.error)
        ...
        try:
            ...
            if not isinstance(data, dict):
                return api_error_response(
                    validation_error("请求体必须是 JSON 对象", "请刷新设置页面后重试。")
                )

            # 更新图片生成配置
            if 'image_generation' in data:
                _update_provider_config(
                    IMAGE_CONFIG_PATH, data['image_generation'], category='image'
                )

            # 更新文本生成配置
            if 'text_generation' in data:
                _update_provider_config(
                    TEXT_CONFIG_PATH, data['text_generation'], category='text'
                )
    ...
    def test_connection():
        ...
        try:
            ...
            if provider_type not in TEXT_PROVIDER_TYPES | IMAGE_PROVIDER_TYPES:
                return api_error_response(
                    validation_error(f"不支持的类型: {provider_type}", "请选择正确的服务商类型后再测试连接。")
                )
            if category not in [None, 'text', 'image']:
                return api_error_response(
                    validation_error("category 必须为 text 或 image", "请刷新设置页面后重试。")
                )
            if category == 'text' and provider_type not in TEXT_PROVIDER_TYPES:
                return api_error_response(
                    validation_error("该类型不能用于文本生成", "请选择文本服务商支持的类型后再测试。")
                )
            if category == 'image' and provider_type not in IMAGE_PROVIDER_TYPES:
                return api_error_response(
                    validation_error("该类型不能用于图片生成", "请选择图片服务商支持的类型后再测试。")
                )
            ...
            if not config['api_key'] and provider_name:
                config = _load_provider_config(provider_type, provider_name, config, category)
...
def _update_provider_config(config_path: Path, new_data: dict, category: str):
    """
    更新服务商配置
    ...
    if not isinstance(new_data, dict):
        raise ConfigValidationError("配置必须是对象", "请刷新设置页面后重试。")
    ...
    if 'active_provider' in new_data:
        active_provider = new_data['active_provider']
        if active_provider is not None and not isinstance(active_provider, str):
            raise ConfigValidationError("active_provider 必须是字符串", "请选择一个服务商后再保存。")
        existing_config['active_provider'] = active_provider or ''

    # 更新 providers
    if 'providers' in new_data:
        new_providers = new_data['providers']
        if not isinstance(new_providers, dict):
            raise ConfigValidationError("providers 必须是对象", "请刷新设置页面后重试。")

        allowed_types = TEXT_PROVIDER_TYPES if category == 'text' else IMAGE_PROVIDER_TYPES
        updated_providers = {}

        for name, submitted_provider in new_providers.items():
            if not isinstance(name, str) or not name.strip():
                raise ConfigValidationError("服务商名称不能为空", "请填写服务商名称后再保存。")
            if not isinstance(submitted_provider, dict):
                raise ConfigValidationError(f"服务商 {name} 的配置格式无效", "请刷新设置页面后重试。")

            # 基于已保存配置合并，避免编辑模型或地址时丢失高级参数。
            new_provider_config = dict(existing_providers.get(name, {}))
            new_provider_config.update(submitted_provider)
            provider_type = new_provider_config.get('type')
            if provider_type not in allowed_types:
                supported = '、'.join(sorted(allowed_types))
                raise ConfigValidationError(
                    f"服务商 {name} 的类型不适用于{ '文本' if category == 'text' else '图片' }生成: {provider_type}",
                    f"请选择以下类型之一：{supported}。",
                )

            # 如果新配置的 api_key 是空的，保留原有的
            submitted_key = submitted_provider.get('api_key')
            if submitted_key in [True, False, '', None]:
                if existing_providers.get(name, {}).get('api_key'):
                    new_provider_config['api_key'] = existing_providers[name]['api_key']
                else:
                    new_provider_config.pop('api_key', None)
            elif not isinstance(submitted_key, str):
                raise ConfigValidationError(f"服务商 {name} 的 API Key 格式无效", "请重新填写 API Key 后保存。")

            # 这些字段只用于前端脱敏显示，绝不写回配置文件。
            new_provider_config.pop('api_key_env', None)
            new_provider_config.pop('api_key_masked', None)
            updated_providers[name] = new_provider_config

        existing_config['providers'] = updated_providers

    active_provider = existing_config.get('active_provider', '')
    if active_provider and active_provider not in existing_config.get('providers', {}):
        raise ConfigValidationError(
            "当前激活的服务商不存在",
            "请先选择一个保留的服务商为激活状态后再保存。",
        )
...
def _load_provider_config(
    ...
    # 兼容旧客户端：OpenAI compatible 默认按文本处理，图片设置页会明确传 category=image。
...
def _test_provider_connection(provider_type: str, config: dict) -> dict:
    ...
    elif provider_type == 'openai_compatible' and _is_image_generation_endpoint(config.get('endpoint_type')):
        # 同一 OpenAI-compatible 类型既用于文本，也用于 gpt-image-2 等图片模型。
        # 图片端点不能发送 chat 请求，改为验证 images API 的可达性。
...
def _is_image_generation_endpoint(endpoint: str | None) -> bool:
    normalized = _normalize_endpoint(endpoint or '')
    return '/images/' in normalized or normalized.endswith('/images')
