...
ERROR_TYPE_BASE = "https://app.local/errors"
...
def classify_error(error: Union[Exception, str], context: Optional[Dict[str, Any]] = None) -> AppError:
    ...
    if _looks_like_upstream_connection_closed(text):
        return AppError(
            code="UPSTREAM_CONNECTION_CLOSED",
            title="图片网关提前断开连接",
            detail=(
                f"{host} 在图片生成完成前关闭了 TLS 连接。"
                if host else "模型网关在图片生成完成前关闭了 TLS 连接。"
            ),
            suggestion="这通常是网关的上游读取超时，不是 Base URL 配错。请检查 API 网关超时设置或稍后重试。",
            status=502,
            retryable=True,
            diagnostics=diagnostics,
        )
...
def _looks_like_upstream_connection_closed(text: str) -> bool:
