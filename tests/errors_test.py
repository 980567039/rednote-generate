...
def test_classifies_upstream_record_layer_failure_without_blame_on_base_url():
    error = classify_error(
        "HTTPSConnectionPool(host='codex.quat.cc', port=443): "
        "Max retries exceeded with url: /v1/images/generations "
        "(Caused by SSLError('[SSL: RECORD_LAYER_FAILURE] record layer failure'))"
    )

    assert error.code == "UPSTREAM_CONNECTION_CLOSED"
    assert error.status == 502
    assert error.retryable is True
    assert "不是 Base URL 配错" in error.suggestion
...
def test_openai_compatible_test_uses_configured_endpoint_for_real_llm_request(monkeypatch):
    ...
    class Response:
        status_code = 200
        text = '{"choices":[{"message":{"content":"连接测试成功"}}]}'

        def json(self):
            return {"choices": [{"message": {"content": "连接测试成功"}}]}
